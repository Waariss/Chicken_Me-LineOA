...
def start_loading(userId):
    ...
    if response.status_code != 200:
        logger.error("Failed to start loading indicator: %s", response.text)
...
# YOLOv5 setup
weights, imgsz = 'best.pt', 640

yolov5_model = None

# Try to load YOLOv5 model
try:
    yolov5_model = torch.hub.load('ultralytics/yolov5', 'custom', path='best.pt', source='github')
except Exception as e:
    logger.error(f"Error loading YOLOv5 model: {e}")

# TensorFlow model for classification
classification_model_path = 'GOOD.keras'
try:
...
# Class names mapping
class_names = {
    0: 'Cocci', 1: 'Healthy', 2: 'Ncd', 3: 'Salmo'
}
...
def handle_content_message(event):
    ...
    for *xyxy, conf, cls in results.xyxy[0]:
        ...
        predicted_label = f'{predicted_class_name}: {np.max(predictions):.2f}'
        annotator.box_label(xyxy, predicted_label, color=colors(int(cls), True))
        full_result_text += f"{predicted_label}\n"
